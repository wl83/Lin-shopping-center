...
def get_current_url(
    ...
    cat(url_quote(wsgi_get_bytes(environ.get)).rstrip("/"))
    cat("/")
    if not root_only:
        cat(url_quote(wsgi_get_bytes(environ.get).lstrip(b"/")))
...
def get_path_info(environ, charset="utf-8", errors="replace"):
    ...
    """
    path = wsgi_get_bytes(environ.get)
...
def peek_path_info(environ, charset="utf-8", errors="replace"):
    ...
    """
    segments = environ.get.lstrip("/").split("/", 1)
    if segments:
        return to_unicode(
...
def wrap_file(environ, file, buffer_size=8192):
    ...
    """
    return environ.get(file, buffer_size)
