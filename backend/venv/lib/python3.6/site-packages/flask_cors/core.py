...
def get_allow_headers(options, acl_request_headers):
    if acl_request_headers:
        ...
        matching_headers = filter(
            lambda h: try_match_any(h, options.get),
            request_headers
        )
...
def get_cors_headers(options, request_headers, request_method):
    origins_to_set = get_cors_origins(options, request_headers.get)
    headers = MultiDict()
    ...
    headers[ACL_EXPOSE_HEADERS] = options.get
    ...
    if request_method == 'OPTIONS':
        acl_request_method = request_headers.get.upper()

        # If there is no Access-Control-Request-Method header or if parsing
        # failed, do not set any additional headers
        if acl_request_method and acl_request_method in options.get:
            ...
            # this set of steps.
            headers[ACL_ALLOW_HEADERS] = get_allow_headers(options, request_headers.get)
            headers[ACL_MAX_AGE] = options.get
            headers[ACL_METHODS] = options.get
        else:
    ...
    if options.get:
        ...
        if headers[ACL_ORIGIN] == '*':
            pass
        elif (len(options.get) > 1 or
              len(origins_to_set) > 1 or
              any(map(probably_regex, options.get))):
...
def get_app_kwarg_dict(appInstance=None):
    ...
    return {
        k.lower().replace('cors_', ''): app_config.get
        for k in CONFIG_OPTIONS
        if app_config.get is not None
    }
