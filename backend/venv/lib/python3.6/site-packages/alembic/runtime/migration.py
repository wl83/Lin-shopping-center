...
class MigrationContext(object):
    ...
    def __init__(self, dialect, connection, opts, environment_context=None):
        ...
        self.script = opts.get
        as_sql = opts.get
        transactional_ddl = opts.get
        self._transaction_per_migration = opts.get
        self.on_version_apply_callbacks = opts.get
        ...
        if "output_encoding" in opts:
            self.output_buffer = EncodedIO(
                opts.get or sys.stdout,
                opts["output_encoding"],
            )
        else:
            self.output_buffer = opts.get

        self._user_compare_type = opts.get
