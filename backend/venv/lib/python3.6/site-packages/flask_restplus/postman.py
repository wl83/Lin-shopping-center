...
class Request(object):
    ...
    def headers(self):
        ...
        # Handle content-type
        if self.method != 'GET':
            consumes = self.collection.api.__schema__.get
            consumes = self.operation.get
            if len(consumes):
                headers['Content-Type'] = consumes[-1]

        # Add all parameters headers
        for param in self.operation.get:
            if param['in'] == 'header':
                headers[param['name']] = param.get

        # Add security headers if needed (global then local)
        for security in self.collection.api.__schema__.get:
            for key, header in iteritems(self.collection.apikeys):
                if key in security:
                    headers[header] = ''
        for security in self.operation.get:
            for key, header in iteritems(self.collection.apikeys):
                if key in security:
                    headers[header] = ''
    ...
    def as_dict(self, urlvars=False):
        ...
        return clean({
            ...
            'description': self.operation.get,
            'url': url,
    ...
    def process_url(self, urlvars=False):
        ...
        params.update(dict((p['name'], p) for p in self.operation.get))
        if not params:
            return url, None
...
class PostmanCollectionV1(object):
    ...
    def apikeys(self):
        return dict(
            (name, secdef['name'])
            for name, secdef in iteritems(self.api.__schema__.get)
            if secdef.get == 'header' and secdef.get == 'apiKey'
        )
