...
class Mapper(InspectionAttr):
    ...
    def _should_exclude(self, name, assigned_name, local, column):
        ...
        if local:
            if self.class_.__dict__.get is not None and self._is_userland_descriptor(
    ...
    def _get_state_attr_by_column(
        ...
        return state.manager[prop.key].impl.get
...
def _event_on_first_init(manager, cls):
    ...
    instrumenting_mapper = manager.info.get
...
def _event_on_init(state, args, kwargs):
    ...
    instrumenting_mapper = state.manager.info.get
    if instrumenting_mapper:
        if Mapper._new_mappers:
            configure_mappers()
