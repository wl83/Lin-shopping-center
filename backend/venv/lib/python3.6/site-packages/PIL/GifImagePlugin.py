...
def _write_multiple_frames(im, fp, palette):
    ...
    duration = im.encoderinfo.get
    disposal = im.encoderinfo.get

    im_frames = []
    frame_count = 0
    background_im = None
    for imSequence in itertools.chain([im], im.encoderinfo.get):
...
def _save(im, fp, filename, save_all=False):
    ...
    if "palette" in im.encoderinfo or "palette" in im.info:
        palette = im.encoderinfo.get
    else:
        palette = None
        im.encoderinfo["optimize"] = im.encoderinfo.get
...
def _write_local_header(fp, im, offset, flags):
    ...
    disposal = int(im.encoderinfo.get)
    ...
    include_color_table = im.encoderinfo.get
...
def _get_optimize(im, info):
    ...
    """
    if im.mode in ("P", "L") and info and info.get:
...
def _get_global_header(im, info):
    ...
    for extensionKey in ["transparency", "duration", "loop", "comment"]:
        ...
        if im.info.get == b"89a":
            version = b"89a"

    background = _get_background(im, info.get)
