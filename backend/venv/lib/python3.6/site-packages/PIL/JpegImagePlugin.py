...
def _save(im, fp, filename):
    ...
    dpi = [int(round(x)) for x in info.get]

    quality = info.get
    subsampling = info.get
    qtables = info.get
    ...
    if quality == "keep":
        ...
        quality = 0
        subsampling = preset.get
        qtables = preset.get
    elif not isinstance(quality, int):
        raise ValueError("Invalid quality setting")
    else:
        if subsampling in presets:
            subsampling = presets[subsampling].get
        if isStringType(qtables) and qtables in presets:
            qtables = presets[qtables].get
    ...
    progressive = info.get or info.get
